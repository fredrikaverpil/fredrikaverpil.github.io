...
- I'm fortunate to work at a company which allows the use of AI, and in this
  case Google Gemini Advanced. It's really, really fast but I really feel Claude
  (Sonnet 3.5) is a lot more accurate as well has better taste. But it's still
  extremely helpful to have this at your disposal. A colleague of mine took a
  third-party API spec and asked Gemini to write the Go types for it for him.
  Awesome!
...
- [gitingest](https://gitingest.com) - essentially replace the `github.com` for
  repos with `gitingest.com` and download the whole repo as one structured text
  file that you can upload to AI chat of your choice. If the text file becomes
  to large for [Claude](https://claude.ai) Sonnet 3.5 (my favorite
  pair-programming LLM), try [Gemini](https://gemini.google.com/), which has
  support for a larger context window.
- [`lazydocker`](https://github.com/jesseduffield/lazydocker) - I Use `lazygit`
  ...
  dashboard but I appreciate not having to leave the terminal.
