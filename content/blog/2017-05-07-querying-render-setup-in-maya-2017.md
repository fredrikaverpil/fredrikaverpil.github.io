...
Again, we can access the `help(function)` and see if that helps revealing useful "callables" (such as methods) by re-using the previously used `get_callable_attributes()` and `print_markdown()` functions (so make sure those are already sourced):
