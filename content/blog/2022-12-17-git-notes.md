...
>
> I have a shim script available on `$PATH` which I run quite often (see my [dotfiles](https://github.com/fredrikaverpil/dotfiles/blob/main/shell/bin/git-reset)) instead of running `git fetch`, `git reset` or `git pull`, just to reset everything for the branch I'm currently on.
>
> ```bash
> #!/bin/sh -e
> 
